...
export const VendorDashboardPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [prodRes, whRes, ordRes] = await Promise.all([
          productService.getProducts().catch(() => []),
          apiClient.get('/inventory/my-warehouse').catch(() => null),
          apiClient.get('/orders/my-orders').catch(() => null)
        ]);
        setProducts(Array.isArray(prodRes) ? prodRes : []);
        if (whRes && whRes.data) {
          setWarehouse(whRes.data.warehouse || whRes.data);
        }
        if (ordRes && ordRes.data && Array.isArray(ordRes.data)) {
  ...
  const monthlySalesSum = orders
    .filter((o) => o.status === 'ENTREGADO' || o.status === 'ENVIADO')
    ...
    <div className="space-y-6">
      ...
      {loading && (
        <div className="flex flex-col items-center justify-center py-12 space-y-3">
          ...
          <p className="text-sm text-slate-500 font-medium">Cargando datos de ventas...</p>
        </div>
      )}
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-5 rounded-2xl flex items-center gap-4 shadow-sm">
              ...
              <div>
                <p className="text-xs text-slate-400 uppercase font-semibold">Ventas Registradas</p>
                <p className="text-2xl font-black text-slate-900 dark:text-white">${monthlySalesSum.toFixed(2)}</p>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-5 rounded-2xl flex items-center gap-4 shadow-sm">
              ...
              <div>
                ...
                <p className="text-2xl font-black text-amber-600 dark:text-amber-400">{pendingOrdersCount} Pedidos</p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm overflow-hidden p-6 space-y-4">
              ...
              {orders.length === 0 ? (
                <div className="p-8 text-center space-y-2 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-dashed border-slate-200 dark:border-slate-700">
                  <FontAwesomeIcon icon={faBoxOpen} className="text-3xl text-slate-300 dark:text-slate-600" />
                  <p className="text-sm font-semibold text-slate-600 dark:text-slate-300">No hay compras registradas en la base de datos.</p>
                  <p className="text-xs text-slate-400">Cuando los clientes compren productos en la tienda, aparecerán listados aquí.</p>
                </div>
              ) : (
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-700/60 text-sm">
                      {orders.slice(0, 5).map((ord) => (
                        <tr key={ord.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/30 transition-colors">
                          ...
                          <td className="p-3 font-medium text-slate-900 dark:text-white">{ord.clientName || 'Cliente'}</td>
                          <td className="p-3 font-bold text-slate-800 dark:text-slate-200">${Number(ord.total || 0).toFixed(2)}</td>
                          ...
                          <td className="p-3 text-right text-xs text-slate-400">{ord.createdAt || 'Hoy'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="space-y-3">
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-bold text-slate-900 dark:text-white text-sm">Mis Productos & Stock</h4>
                      <p className="text-xs text-slate-400">Revisar catálogo y solicitar reposición</p>
                    </div>
                  </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-bold text-slate-900 dark:text-white text-sm">Órdenes de Venta</h4>
                      <p className="text-xs text-slate-400">Atender pedidos recibidos</p>
                    </div>
                  </div>
